...
class UEAloader(Dataset):
    ...
    def load_single(self, filepath):
        ...
        ## (Below) Fix for CharacterTrajectories (all Channels (Columns) does not have equal length)
        new_dfs = []
        for row in range(df.shape[0]):
            # get all series for this sample
            series_list = [df.loc[row, col] for col in df.columns]

            # find max length across dimensions
            max_len = max(len(s) for s in series_list)

            padded_data = {}
            for col, s in zip(df.columns, series_list):
                s = np.array(s)

                if len(s) < max_len:
                    # pad with NaN (better than 0 for interpolation later)
                    s = np.pad(s, (0, max_len - len(s)), constant_values=np.nan)

                padded_data[col] = s

            tmp_df = pd.DataFrame(padded_data)
            tmp_df.index = [row] * max_len

            new_dfs.append(tmp_df)
        df = pd.concat(new_dfs, axis=0)
        ## (Above) Fix for CharacterTrajectories (all Channels (Columns) does not have equal length)
...
class FLAAPLoader(Dataset):
    ...
    def load_flaap_dependent(self, data_path, label_path, flag=None):
        ...
        X_train = np.load(data_path)
        y_train = np.load(label_path)
        # print("Train Size:", "X Size:", X_train.shape, "y Size:", y_train.shape)

        # 60 : 20 : 20
        X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, test_size=0.2, random_state=42)
        X_train, X_test, y_train, y_test = train_test_split(X_train, y_train, test_size=0.2, random_state=42)
        # test_size=0.25 was in the original code (but in the paper SGN authors said test_size was 0.2 instead)
        
        # print("Test Size:", "X Size:", X_test.shape, "y Size:", y_test.shape)
        # print("Val Size:", "X Size:", X_val.shape, "y Size:", y_val.shape)
