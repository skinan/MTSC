...
class Exp_Classification(Exp_Basic):
    ...
    def _build_model(self):
        ...
        other_data = ['UCIHAR', "FLAAP"]
        if self.args.data in other_data:
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        f1_score = cal_f1_score(predictions, trues)
    ...
    def train(self, setting):
        train_data, train_loader = self._get_data(flag='TRAIN')
        vali_data, vali_loader = self._get_data(flag='VAL') # UCI-HAR=VAL & UEA=TEST
        test_data, test_loader = self._get_data(flag='TEST')
        ...
        for epoch in range(self.args.train_epochs):
            ...
            else: # Following SGN's Experimental Protocol
    ...
    def test(self, setting, test=0):
        ...
        folder_path = f'./results/{self.args.data}/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        ...
        file_name=f'{self.args.model_id}.csv'
        ...
        with open(file_path, mode='a', newline='') as f:
            ...
            if not file_exists:
                writer.writerow(['MODEL', 'ACCURACY', 'NOTE'])

            # append new row
            writer.writerow([self.args.model, f"{accuracy:.4f}", self.args.note])
